...
class GeminiCoderWebApp(GeminiCoderApp):
    ...
    def _build_settings_view(self) -> None:
        ...
        ctk.CTkLabel(
            scroll,
            text="Connect ONE Gemini window. Autocode builds and endlessly improves code.",
            font=("Segoe UI", 12),
        ...
        # Hidden cards for other corners (kept so session_mgr doesn't break)
        # They're never shown in the UI but exist as empty references
        card_tr = SessionCard(top_row, "top-right", c, self)
        self._session_cards["top-right"] = card_tr

        # Hidden bottom row cards (reserved for future four-way mode)
        ...
        ctk.CTkLabel(
            bc_card, text=(
                ...
                "Gemini builds the code, then endlessly improves it until you click Stop.\n"
    ...
    def _on_broadcast_start(self) -> None:
        ...
        if not active:
            self._toast("No configured session. Launch or capture a Gemini window first.", "error")
            return
    ...
    def _on_launch_cdp_browser(self) -> None:
        ...
        def worker():
            ...
            # Auto-grab: connect the single Gemini tab to top-left
            from ..cdp_client import (
                CDPConnection, CDPChatAutomation, get_selectors_for_profile,
                claim_ws_url, release_ws_url, _find_unclaimed_target,
            )

            target = _find_unclaimed_target("gemini.google.com", "", 9222)
            if not target:
                self.after(0, lambda: self._cdp_status.configure(
                    text="Chrome launched but no Gemini tab found. Open gemini.google.com manually.",
                    text_color=self._colors["warning"],
                ))
                return
            ...
            selectors = get_selectors_for_profile("Gemini")
            automation = CDPChatAutomation(conn, selectors, "Gemini")
            profile = get_profile("Gemini")

            card = self._session_cards.get("top-left")
            if card:
                if not card._session:
                    try:
                        session = self.session_mgr.create_session(profile, "top-left")
                        card._session = session
                        card._wire_callbacks()
                    except RuntimeError as e:
                ...
                if card._session and card._session.client:
                    ...
                    def _update_card(c=card):
                        c._set_state("ready")
                        c._mode_label.configure(
                            text="Mode: CDP (reliable)",
                            text_color=c._colors["success"])
                        c._ai_selector.set("Gemini")
                    self.after(0, _update_card)
                    logger.info("Auto-grabbed Gemini CDP tab for top-left")

            msg = f"{ICONS['check']} CDP active! Gemini session connected."
            self.after(0, lambda: self._cdp_status.configure(
                text=msg, text_color=self._colors["success"],
            ))
            self.after(0, lambda: self._toast("CDP Browser ready! Gemini connected.", "success"))
